...
```bash
python3 utils/capture_pose_data.py --pose_name="[THE NAME OF THE ACTION]" --confidence=[THE CONFIDENCE OF THE POSE ESTIMATON MODEL (TYPICALLY 0.5)] --duration=[DATA COLLECTION PERIOD (TYPICALLY 60)]
```
5) (OPTIONAL) After collecting data for all the actions you want, train the model using the command
```bash
python3 utils/train.py --model_name=[NAME OF THE MODEL YOU WANT] 
```
